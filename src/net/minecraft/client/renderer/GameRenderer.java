...
import minecraft.game.operation.combat.DDENTITYTR;
import minecraft.game.operation.misc.AspectRatio;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            DDENTITYTR DDENTITYTR = moduleManager.getDDENTITYTR();

            if (!DDENTITYTR.isEnabled()) {
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            ModuleManager moduleManager = AG.getInst().getModuleManager();
            DDBETTER DDBETTER = moduleManager.getDDBETTER();
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        if (!AG.getInst().getModuleManager().getSelfDestruct().unhooked) {
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks,
        ...
        ModuleManager moduleRepository = AG.getInst().getModuleManager();
        ...
        matrixstack.getLast().getMatrix()
                .mul(Matrix4f
                        .perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting),
                                aspectRatio.isEnabled() ? (float)aspectRatio.width.getValue().floatValue() : (float)this.mc.getMainWindow().getFramebufferWidth() / (float)this.mc.getMainWindow().getFramebufferHeight(), 0.05f, this.clipDistance));
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        if (this.mc.player !=null) {
            f = MathHelper.lerp(partialTicks, this.mc.player.prevTimeInPortal, this.mc.player.timeInPortal)
                    * this.mc.gameSettings.screenEffectScale * this.mc.gameSettings.screenEffectScale;
        }
