...
public class RotationHandler implements IMinecraft {
    ...
    private static boolean updateRotation(Rotation rotation, float turnSpeed) {
        ...
        mc.player.rotationYaw += GCDSensSystem.getSensitivity(clamp(yawDelta, -yawSpeed, yawSpeed));
        mc.player.rotationPitch = clamp(mc.player.rotationPitch + GCDSensSystem.getSensitivity(clamp(pitchDelta, -pitchSpeed, pitchSpeed)), -90, 90);
