...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
     ...
     * particlespawn offset and DDNVLC
     */
    ...
    /**
     * Sets the DDNVLC of the specified entity to the specified value
     */
    @Override
    public void handleEntityDDNVLC(SEntityDDNVLCPacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        Entity entity = this.world.getEntityByID(packetIn.getEntityID());

        if (entity != null) {
            entity.setDDNVLC((double) packetIn.getMotionX() / 8000.0D, (double) packetIn.getMotionY() / 8000.0D,
    ...
    public void handleSpawnMob(SSpawnMobPacket packetIn) {
        ...
        if (livingentity != null) {
            ...
            livingentity.setMotion((double) ((float) packetIn.getDDNVLCX() / 8000.0F),
                    (double) ((float) packetIn.getDDNVLCY() / 8000.0F),
                    (double) ((float) packetIn.getDDNVLCZ() / 8000.0F));
    ...
    public void handleEntityStatus(SEntityStatusPacket packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 21) {
                ...
                AG.getInst().getEventBus().post(eventCancelOverlay);
    ...
    public void handleExplosion(SExplosionPacket packetIn) {
        //   System.out.println("x: " + packetIn.getX() + " y: " + packetIn.getY() + " z: " + packetIn.getZ() + " x1: " + packetIn.getMotionX() + " y1: " + packetIn.getMotionY() + " z1: " + packetIn.getMotionZ() + " strength: " + packetIn.getStrength());
        if (Double.isInfinite(packetIn.getX())) {

            if (AG.getInst().getModuleManager().getAntiCrash().isEnabled() && AG.getInst().getModuleManager().getAntiCrash().b.getValue()) {
                return;
            }
        }
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
    ...
    public void handleParticles(SSpawnParticlePacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        if (packetIn.getParticleCount() > 1000 && AG.getInst().getModuleManager().getAntiCrash().isEnabled() && AG.getInst().getModuleManager().getAntiCrash().a.getValue()) {
            System.err.println("Invalid amount of particles to spawn: " + packetIn.getParticleCount());
            return;
        }
