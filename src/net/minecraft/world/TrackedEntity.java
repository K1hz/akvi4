...
public class TrackedEntity
{
    ...
    private final boolean sendDDNVLCUpdates;
    ...
    private Vector3d DDNVLC = Vector3d.ZERO;
    ...
    public TrackedEntity(ServerWorld serverWorld, Entity entity, int updateFrequency, boolean sendDDNVLCUpdates, Consumer < IPacket<? >> packetConsumer)
    {
        this.world = serverWorld;
        this.packetConsumer = packetConsumer;
        this.trackedEntity = entity;
        this.updateFrequency = updateFrequency;
        this.sendDDNVLCUpdates = sendDDNVLCUpdates;
    ...
    public void tick()
    {
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataManager().isDirty())
        {
            ...
            else
            {
                ...
                if ((this.sendDDNVLCUpdates || this.trackedEntity.isAirBorne || this.trackedEntity instanceof LivingEntity && ((LivingEntity)this.trackedEntity).isElytraFlying()) && this.updateCounter > 0)
                {
                    Vector3d vector3d1 = this.trackedEntity.getMotion();
                    double d0 = vector3d1.squareDistanceTo(this.DDNVLC);

                    if (d0 > 1.0E-7D || d0 > 0.0D && vector3d1.lengthSquared() == 0.0D)
                    {
                        this.DDNVLC = vector3d1;
                        this.packetConsumer.accept(new SEntityDDNVLCPacket(this.trackedEntity.getEntityId(), this.DDNVLC));
                    }
                }
        ...
        if (this.trackedEntity.DDNVLCChanged)
        {
            this.sendPacket(new SEntityDDNVLCPacket(this.trackedEntity));
            this.trackedEntity.DDNVLCChanged = false;
        }
    }
    ...
    public void sendSpawnPackets(Consumer < IPacket<? >> packetConsumer)
    {
        ...
        boolean flag = this.sendDDNVLCUpdates;
        ...
        this.DDNVLC = this.trackedEntity.getMotion();

        if (flag && !(ipacket instanceof SSpawnMobPacket))
        {
            packetConsumer.accept(new SEntityDDNVLCPacket(this.trackedEntity.getEntityId(), this.DDNVLC));
        }
