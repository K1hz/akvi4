...
@Defuse(name = "Flight",description = "Полёт", brand = Category.Movement)
public class Fly extends Module {
    ...
    public CheckBoxSetting noVanillaKick = new CheckBoxSetting("Vanilla DDDSLB", true).visibleIf(() -> mode.is("MatrixChunk"));
    ...
    TimeCounterSetting timeFlying = new TimeCounterSetting();

    @Subscribe
    public void onUpdate(EventUpdate e) {
        if (mc.player == null || mc.world == null || AG.getInst().getModuleManager().getDDFRCM().isEnabled())
            return;

        if (options.is("Авто прыжок").getValue() && mc.player.isOnGround() && mode.is("BlockPlace") && !MovementSystem.isMoving()) {
            mc.player.jump();
        }
        ...
        switch (mode.getValue()) {
            ...
            case "NCP" -> {
                ...
                boolean move = MovementSystem.moveKeysPressed();
                double speed = up || down ? 0.0 : (move ? MovementSystem.getSpeedByBPS(4.010000000000001) : 0.0);
                float moveYaw = (float)Math.toRadians(MovementSystem.moveYaw(mc.player.rotationYaw));
                ...
                MovementSystem.setMotionSpeed(true, false, 0.0);
                MovementSystem.setSpeed((float) (-speed / 5.0));
            ...
            case "MatrixChunk" -> {
                MovementSystem.setSpeed((float) (smoothSpeed.getValue() ? MovementSystem.getSpeed() / 5.0 : MovementSystem.getSpeed() / 30.0), smoothSpeed.getValue() ? 0.3F : 0.0F);
            }
            ...
            case "MatrixJump" -> { // Matrix Jump
                if (mc.player.isOnGround())
                    ...
                    MovementSystem.setMotion(Math.min(horizontal.getValue(), 1.97f));
                    mc.player.motion.y = vertical.getValue();
                }
            }

            case "MatrixGlide" -> { // Matrix Glide
                MovementSystem.setMotion(horizontal.getValue());
            ...
            case "BlockPlace" -> {
                ...
                if (speedMode.is("Motion") && mc.player.isOnGround()) {
                    MovementSystem.setMotion(motionSpeed.getValue());
                }
                BlockPos posBelow;
                if (!mc.world.getBlockState(posBelow = mc.player.getPosition().down()).getMaterial().isReplaceable()) break;

                placeBlockUnderPlayer(posBelow, (MovementSystem.isMoving() ? 0.2f : 0.1f));
            }
            ...
            case "AAC" -> {
                ...
                if (MovementSystem.getSpeed() > 0.19) {
    ...
    public void onMove(EventMoving e) {
        ...
        if (mode.is("MatrixChunk") && mc.player.fallDistance != 0.0F) {
            ...
            motion = smoothSpeed.getValue() ? MovementSystem.getCuttingSpeed() / 1.3 : 0.0;
            if (mc.player.fallDistance != 0.0F || !autoUp.getValue()) {
                motion = (mc.player.isJumping && upWard.getValue() ? 8.2675F : 9.4675F) * curMotion;
            }

            if (!MovementSystem.moveKeysPressed() && smoothSpeed.getValue()) {
                MovementSystem.setSpeed((float) motion, 0.6F);
            } else {
                MovementSystem.setCuttingSpeed(motion / 1.06);
                MovementSystem.setSpeed((float) motion);
            }
        }
    }
    ...
    public void onMotion(EventMotion e) {
        ...
        if (mode.is("MatrixChunk")) {
            ...
            if (mc.player.fallDistance != 0.0F) {
                ...
                if (mc.player.isJumping && this.upWard.getValue() && MovementSystem.moveKeysPressed() && MovementSystem.getCuttingSpeed() > 0.1) {
    ...
    public static void placeBlockUnderPlayer(BlockPos pos, float fallDistance) {
        int blockSlot = InventoryOrigin.findBlockInHotbar();
        if (blockSlot == -1) {
            return;
        }
