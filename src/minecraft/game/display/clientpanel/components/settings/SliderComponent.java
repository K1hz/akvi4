...
public class SliderComponent extends Component {
    ...
    public void render(MatrixStack stack, float mouseX, float mouseY) {
        ...
        GraphicsSystem.drawRoundedRect(getX() + 5, getY() + 11, getWidth() - 10, 6, 2f, ColoringSystem.rgba(16,16,16, 26));

        anim = MathSystem.fast(anim, (getWidth() - 10) * (setting.getValue() - setting.min) / (setting.max - setting.min), 20);
        float sliderWidth = anim;

        GraphicsSystem.drawRoundedRect(getX() + 4.5f, getY() + 11, sliderWidth, 6, 2, new Vector4i(
