...
import static com.jhlabs.math.Noise.lerp;
import static net.minecraft.util.math.MathHelper.wrapDegrees;

@Getter
@Defuse(name = "Legit Aurа", description = "PD", brand = Category.Combat)
public class LegitATTACK extends Module {
    ...
    public final SliderSetting postionY = new SliderSetting("Позиция наводки",
            0f, -1f, 1f, 0.1f
    );
    ...
    public final CheckBoxSetting onlyCrits = new CheckBoxSetting("Только криты", true);
    public final CheckBoxSetting accelerateRotation = new CheckBoxSetting("Ускорять ротацию", false);
    public final CheckBoxSetting tpsSync = new CheckBoxSetting("Синхронизация с тиками", true);
    public final CheckBoxSetting targetFocus = new CheckBoxSetting("Фокусировать одну цель", false);
    public final CheckBoxSetting wallsAttack = new CheckBoxSetting("Бить через стены", false);
    ...
    public final SliderSetting radiusFov = new SliderSetting("Фов",
            90, 10, 180, 0.5f
    ).visibleIf(() -> this.fov.getValue());
    public final ModeSetting sprintType = new ModeSetting("Режим спринта",
            "Обычный",
            "Обычный",
            "Легитный"
    );
    ...
    private final long UPDATE_INTERVAL = 200;
    private float targetYOffset = 0.35f;
    private float currentYOffset = 0.35f;
    ...
    public LegitATTACK() {
        this.addSettings(this.attackRange, this.assistStrength, this.smoothFactor, this.yawSpeed, this.pitchSpeed, this.postionY, this.typeFilter, this.targetsSort, this.onlyCrits, this.accelerateRotation, this.tpsSync, this.targetFocus, this.wallsAttack, this.fov, this.sprintReset, this.radiusFov, this.noAttackCheck);
    }
    @Subscribe
    ...
    private void setPlayerRotation(LivingEntity target, float yawSpeed, float pitchSpeed, float smoothFactor) {
        ...
        Vector3d targetVec = target.getPositionVec().add(0, target.getEyeHeight() / 2.0 + this.postionY.getValue(), 0);
    ...
    private void updateAttack() {
        ...
        this.timerUtil.setLastMS(500L);
        DDATTACK.mc.playerController.attackEntity(DDATTACK.mc.player, target);
    ...
    public boolean shouldPlayerFalling() {
        ...
        float adjustTicks = tpsSync.getValue() ? AG.getInst().getTpsCalc().getAdjustTicks() : 1.5F;

        if (mc.player.getCooledAttackStrength(adjustTicks) < 0.92F) {
            return false;
        }

        if (!cancelReason && onlyCrits.getValue()) {
    ...
    private boolean isValid(LivingEntity entity) {
        ...
        if (!wallsAttack.getValue() && !mc.player.canEntityBeSeen(entity)) {
            return false;
        }
    ...
    public LivingEntity getTarget() {
        return AG.getInst().getModuleManager().getLegitATTACK().isEnabled() ? AG.getInst().getModuleManager().getLegitATTACK().target : null;
    }
}