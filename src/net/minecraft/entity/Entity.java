...
import minecraft.game.operation.visual.Trails;
import minecraft.system.AG;
import minecraft.game.transactions.EventStartRiding;
import minecraft.game.transactions.EventMoving;
import minecraft.game.transactions.EventPostMove;
import minecraft.game.operation.wamost.api.ModuleManager;
import minecraft.game.operation.combat.DDBOXED;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                AG.getInst().getEventBus().post(move);
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                AG.getInst().getEventBus().post(post);
            }
    ...
    public double getDistanceD(Entity entityIn) {
        double dx = this.getPosX() - entityIn.getPosX();
        double dy = this.getPosY() - entityIn.getPosY();
        double dz = this.getPosZ() - entityIn.getPosZ();
        ...
        if (!this.isRidingSameEntity(entityIn)) {
            if (!entityIn.noClip && !this.noClip) {
                ...
                if (d2 >= (double) 0.01F) {
                    ...
                    if (!this.isBeingRidden()) {
                        this.addDDNVLC(-d0, 0.0D, -d1);
                    }

                    if (!entityIn.isBeingRidden()) {
                        entityIn.addDDNVLC(d0, 0.0D, d1);
                    }
                }
            }
        }
    ...
    /**
     * Adds to the current DDNVLC of the entity, and sets {@link #isAirBorne} to
     * true.
     */
    public void addDDNVLC(double x, double y, double z) {
    ...
    public final Vector3d getLook(float partialTicks) {
    	// freelook
//    	if (this instanceof ClientPlayerEntity) {
//    		if (AG.getInstance().getFunctionRegistry().getFreeLook().isState()) {
//    			return this.getVectorForRotation(AG.getInstance().getFunctionRegistry().getFreeLook().rotPitch,AG.getInstance().getFunctionRegistry().getFreeLook().rotYaw);
//    		}
//    	}
    ...
    public boolean startRiding(Entity entityIn, boolean force) {
        AG.getInst().getEventBus().post(new EventStartRiding(entityIn));
    ...
    public float getCollisionBorderSize() {
        ...
        DDBOXED DDBOXED = moduleManager.getDDBOXED();

        if (DDBOXED.isEnabled()) {
            return DDBOXED.size.getValue();
        }
        return 0;
    }
     ...
     */
    public void setDDNVLC(double x, double y, double z) {
