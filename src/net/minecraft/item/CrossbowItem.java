...
public class CrossbowItem extends ShootableItem implements IVanishable
{
    ...
    private static void fireProjectile(World worldIn, LivingEntity shooter, Hand handIn, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean isCreativeMode, float DDNVLC, float inaccuracy, float projectileAngle)
    {
        if (!worldIn.isRemote)
        {
            ...
            else
            {
                ...
                projectileentity.shoot((double)vector3f.getX(), (double)vector3f.getY(), (double)vector3f.getZ(), DDNVLC, inaccuracy);
            }
    ...
    public static void fireProjectiles(World worldIn, LivingEntity shooter, Hand handIn, ItemStack stack, float DDNVLCIn, float inaccuracyIn)
    {
        ...
        for (int i = 0; i < list.size(); ++i)
        {
            ...
            if (!itemstack.isEmpty())
            {
                if (i == 0)
                {
                    fireProjectile(worldIn, shooter, handIn, stack, itemstack, afloat[i], flag, DDNVLCIn, inaccuracyIn, 0.0F);
                }
                else if (i == 1)
                {
                    fireProjectile(worldIn, shooter, handIn, stack, itemstack, afloat[i], flag, DDNVLCIn, inaccuracyIn, -10.0F);
                }
                else if (i == 2)
                {
                    fireProjectile(worldIn, shooter, handIn, stack, itemstack, afloat[i], flag, DDNVLCIn, inaccuracyIn, 10.0F);
