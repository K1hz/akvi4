...
public class IngameGui extends AbstractGui {
    ...
    public TimeCounterSetting watch = new TimeCounterSetting();
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (eventRender2D.getType() == EventRender2D.Type.PRE) {
            AG.getInst().getEventBus().post(eventRender2D);
        }
    ...
    protected void renderHotbar(float partialTicks, MatrixStack matrixStack) {
        boolean isHudEnabled = AG.getInst().getModuleManager().getHud().isEnabled();
        boolean isHotbarEnabled = AG.getInst().getModuleManager().getHud().elements.is("HotBar").getValue();

        if (!isHudEnabled || !isHotbarEnabled) {
