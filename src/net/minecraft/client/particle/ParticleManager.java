...
//        if (DDBETTER.isFpsMode() || AG.getInst().getModuleManager().getNoRender().isEnabled() && AG.getInst().getModuleManager().getNoRender().element.is("Партиклы").getValue()) return;
// Получаем экземпляр DDBETTER
        DDBETTER DDBETTER = AG.getInst().getModuleManager().getDDBETTER();

// Проверяем, активен ли режим FPS или выключен ли рендер для партиклей
        boolean fps = DDBETTER.isFpsMode() ||
                (AG.getInst().getModuleManager().getNoRender().isEnabled() &&
                        AG.getInst().getModuleManager().getNoRender().element.is("Партиклы").getValue());

// Включаем световой режим для частиц
    ...
    public void addBlockHitEffects(BlockPos pos, Direction side)
    {
        ...
        if (blockstate.getRenderType() != BlockRenderType.INVISIBLE)
        {
            ...
            this.addEffect((new DiggingParticle(this.world, d0, d1, d2, 0.0D, 0.0D, 0.0D, blockstate)).setBlockPos(pos).multiplyDDNVLC(0.2F).multiplyParticleScaleBy(0.6F));
        }
    }
