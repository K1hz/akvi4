...
public class FirstPersonRenderer {
    ...
    private void transformSideFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float equippedProg) {
        ...
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        if (viewModel.DDONLY.getValue() && DDATTACK.getTarget() == null && viewModel.isEnabled() && viewModel.swingAnim.getValue()) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (viewModel.isEnabled()) {
                    if (flag3) {
                        if (viewModel.vmeste.getValue()) {
                            matrixStackIn.translate(viewModel.x.getValue(),
                                    viewModel.y.getValue(), viewModel.z.getValue());
                        }
                        if (!viewModel.vmeste.getValue()) {
                            matrixStackIn.translate(viewModel.right_x.getValue(),
                                    viewModel.right_y.getValue(), viewModel.right_z.getValue());
                        }
                    } else {
                        if (viewModel.vmeste.getValue()) {
                            matrixStackIn.translate(viewModel.x.getValue(),
                        ...
                        if (!viewModel.vmeste.getValue()) {
                            matrixStackIn.translate(viewModel.left_x.getValue(),
                                    viewModel.left_y.getValue(), viewModel.left_z.getValue());
                        }
                    }
                }
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
