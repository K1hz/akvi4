...
public class Font implements IMinecraft {
    ...
    public void drawText(MatrixStack stack, String text, float x, float y, int color, float size) {
        ...
        ShaderModules shader = ShaderModules.textShader;
        ...
        shader.setUniform("color", ColoringSystem.rgba(color));
    ...
    public void drawText(MatrixStack stack, ITextComponent text, float x, float y, float size, int alpha) {
        ...
        for (ITextComponent it : text.getSiblings()) {
            ...
            for (ITextComponent it1 : it.getSiblings()) {
                ...
                if (it1.getStyle().getColor() != null) {
                    drawText(stack, draw, x + offset, y, ColoringSystem.setAlpha(ColoringSystem.toColor(it1.getStyle().getColor().getHex()), alpha), size);
                } else
                    drawText(stack, draw, x + offset, y, ColoringSystem.setAlpha(Color.GRAY.getRGB(), alpha), size);
                offset += getWidth(draw, size);
            }
            if (it.getSiblings().size() <= 1) {
                String draw = TextFormatting.getTextWithoutFormattingCodes(it.getString());

                drawText(stack, draw, x + offset, y, ColoringSystem.setAlpha(it.getStyle().getColor() == null ? Color.GRAY.getRGB() : it.getStyle().getColor().getColor(), alpha), size);
                offset += getWidth(draw, size);
            }
        ...
        if (text.getSiblings().isEmpty()) {
            ...
            drawText(stack, draw, x + offset, y, ColoringSystem.setAlpha(text.getStyle().getColor() == null ? Color.GRAY.getRGB() : text.getStyle().getColor().getColor(), alpha), size);
            offset += getWidth(draw, size);
        }
    }
