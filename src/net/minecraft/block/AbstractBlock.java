...
public abstract class AbstractBlock {
    ...
    /**
     * Determines how much DDNVLC is maintained while moving on top of this block
     */
    ...
    public float getPlayerRelativeBlockHardness(BlockState state, PlayerEntity player, IBlockReader worldIn, BlockPos pos) {
        ...
        if (f == -1.0F) {
            ...
            return (autoTool.isEnabled() && autoTool.silent.getValue() && autoTool.itemIndex != -1 ? player.getDigSpeed(state, Minecraft.getInstance().player.inventory.getStackInSlot(autoTool.itemIndex), autoTool.itemIndex) : player.getDigSpeed(state)) / f / (float) i;
        }
    }
    ...
    public abstract static class AbstractBlockState extends StateHolder<Block, BlockState> {
        ...
        public boolean causesSuffocation(IBlockReader worldIn, BlockPos pos) {
            ...
            if (DDPUSH.isEnabled()
                    && DDPUSH.modes.is("Блоки").getValue()) {
                return false;
            }
