...
import minecraft.game.operation.combat.DDEXPLOSION;
import minecraft.game.operation.misc.DDPUSH;
import minecraft.game.operation.movement.Scaffold;
import minecraft.game.operation.visual.ViewModel;
...
public abstract class LivingEntity extends Entity {
    ...
    /**
     * Applies a DDNVLC to the entities, to push them away from eachother.
     */
    @Override
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity entity) {
            AG.getInst().getEventBus().post(eventJump);
        }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                DDEXPLOSION DDEXPLOSION = AG.getInst().getModuleManager().getDDEXPLOSION();
                Scaffold scaffold = AG.getInst().getModuleManager().getScaffold();
                boolean DDATTACKIsNull = DDATTACK.getTarget() == null || !DDATTACK.isEnabled();

                // DDATTACK
                float f = this.rotationPitch * ((float) Math.PI / 180F);
                if (AG.getInst().getModuleManager().getPeregon().isEnabled()) {
                    if ((Boolean) DDATTACK.getOptions().is("Коррекция движения").getValue() && DDATTACK.getTarget() != null && this instanceof ClientPlayerEntity) {
                        vector3d1 = this.getVectorForRotation(DDATTACK.rotateVector.y, DDATTACK.rotateVector.x);
                    }
                if (DDATTACK.isEnabled() && DDATTACK.getOptions().is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                        f = DDATTACK.rotateVector.y * ((float) Math.PI / 180F);
                    }
                }

                // DDEXPLOSION
                if (DDEXPLOSION.check() && DDEXPLOSION.isEnabled() && DDATTACKIsNull && DDEXPLOSION.options.is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(DDEXPLOSION.rotationVector.y, DDEXPLOSION.rotationVector.x);
                }

                // scaffold
                if (scaffold.check() && scaffold.isEnabled() && DDATTACKIsNull && this instanceof ClientPlayerEntity && scaffold.scaffmode.is("Normal")) {
    ...
    public Vector3d getPositon(float partialTicks) {
        return MathSystem.interpolate(getPositionVec(), new Vector3d(lastTickPosX, lastTickPosY, lastTickPosZ), partialTicks);
    }
    ...
    public boolean canBePushed() {
        ...
        if (DDPUSH.isEnabled() && DDPUSH.modes.is("Игроки").getValue() && this instanceof ClientPlayerEntity) {
            return false;
        }
    ...
    /**
     * Marks this entity's DDNVLC as changed, so that it can be re-synced with the
     * client later
     */
    @Override
    protected void markDDNVLCChanged() {
        this.DDNVLCChanged = this.rand.nextDouble() >= this.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE);
    }
