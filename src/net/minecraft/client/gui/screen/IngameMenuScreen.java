...
public class IngameMenuScreen extends Screen {
    ...
    private void addButtons() {
        ...
        if (!SelfDestruct.unhooked && !Minecraft.getInstance().isSingleplayer()) {
        ...
        if (!SelfDestruct.unhooked) {
            this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 151, 98, 20, new TranslationTextComponent("Telegram"), (button2) -> {
                try {
                    Util.getOSType().openURI(new java.net.URI("https://t.me/aegisclient"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }));
            this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 151, 98, 20, new TranslationTextComponent("Discord"), (button2) -> {
                try {
                    Util.getOSType().openURI(new java.net.URI("https://discord.gg/aegisclient"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }));
