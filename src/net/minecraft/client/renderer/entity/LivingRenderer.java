...
import minecraft.game.operation.visual.WorldTweaks;
...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M> {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn)) {
            ...
            boolean flag1 = this.isVisible(entityIn);
            boolean flag2 = !flag1 && !entityIn.isInvisibleToPlayer(minecraft.player);
            boolean flag3 = minecraft.isEntityGlowing(entityIn) && !AG.getInst().getModuleManager().getNoRender().isEnabled() && !AG.getInst().getModuleManager().getNoRender().element.is("Эффект свечения").getValue();
            ...
            if (rendertype != null) {
                ...
                float green = 1F;
                float blue = 1F;
                if (entityIn instanceof PlayerEntity && AG.getInst().getModuleManager().getHitColor().isEnabled() && entityIn.hurtTime > 0) {
                    float hurtPC = (float) Math.sin(entityIn.hurtTime * (18F * Math.PI / 180F));
                    float[] rgb = ColoringSystem.getRGBf(ColoringSystem.overCol(ColoringSystem.getColor(red, green, blue), AG.getInst().getModuleManager().getHitColor().color().getValue(), hurtPC));
                    red = rgb[0];
                    green = rgb[1];
                    blue = rgb[2];
                }

                this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, red, green, blue, 1.0F);
            }
