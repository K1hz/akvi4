...
import minecraft.system.capes.math.Vector2;
import minecraft.system.capes.math.Vector3;
...
public interface CapeHolder {
    ...
    default void simulate(AbstractClientPlayerEntity abstractClientPlayer) {
        ...
        float heightMul = (float) CapesBase.config.heightMultiplier;
        final float straveMul = (float) CapesBase.config.straveMultiplier;
        ...
        if (abstractClientPlayer.canSwim()) {
            simulation.setGravity(CapesBase.config.gravity / 10.0f);
        } else {
            simulation.setGravity((float) CapesBase.config.gravity);
        }
