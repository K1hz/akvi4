...
public abstract class PlayerEntity extends LivingEntity {
    ...
    public void tick() {
        ...
        AG.getInst().getEventBus().post(livingUpdate);
    ...
    public boolean func_234569_d_(BlockState p_234569_1_) {
        AutoTool autoTool = AG.getInst().getModuleManager().getAutoTool();
        return !p_234569_1_.getRequiresTool() || (autoTool.isEnabled() && autoTool.silent.getValue() && autoTool.itemIndex != -1
     ...
     * @return
     */
    public boolean attackTargetEntityWithCurrentItem(Entity targetEntity) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            if (targetEntity instanceof LivingEntity) {
                                ...
                                targetEntity.addDDNVLC((double) (-MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)) * (float) i * 0.5F), 0.1D, (double) (MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)) * (float) i * 0.5F));
                            }
                        ...
                        if (targetEntity instanceof ServerPlayerEntity && targetEntity.DDNVLCChanged) {
                            ((ServerPlayerEntity) targetEntity).connection.sendPacket(new SEntityDDNVLCPacket(targetEntity));
                            targetEntity.DDNVLCChanged = false;
    ...
    public boolean isPushedByWater() {
        ...
        if (DDPUSH.isEnabled() && DDPUSH.modes.is("Вода").getValue() && this instanceof ClientPlayerEntity) {
            return false;
        }
        return !this.abilities.isFlying;
    }
    ...
    public float getCooledAttackStrength(float adjustTicks) {
        return MathHelper.clamp(((float) this.ticksSinceLastSwing + adjustTicks) / this.getCooldownPeriod(), 0.0F, 1F);
    }
