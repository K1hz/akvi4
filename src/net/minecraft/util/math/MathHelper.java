...
public class MathHelper
{
    ...
    public static double wrapDegrees1(double value) {
        // Обычное приведение угла в диапазон от -180 до 180
        double d0 = value % 360.0D;

        // Плавное поведение для углов
        if (d0 > 180.0D) {
            d0 -= 360.0D;
        } else if (d0 < -180.0D) {
            d0 += 360.0D;
    ...
    public static double smoothWrap(double current, double target, double smoothingFactor) {
        // Вычисляем минимальный угол между текущим и целевым
        double delta = wrapDegrees(target - current);

        // Применяем плавное изменение, используя интерполяцию для минимизации рывков
        double smoothDelta = delta * smoothingFactor; // Множитель для плавности

        return current + smoothDelta;
    }
