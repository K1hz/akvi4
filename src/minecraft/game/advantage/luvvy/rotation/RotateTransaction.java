...
import minecraft.game.advantage.figures.Mathf;
...
@Data
@UtilityClass
public class RotateTransaction implements IMinecraft {
...
//
//        double fixedsens = mc.gameSettings.mouseSensitivity - (mc.gameSettings.mouseSensitivity - 0.02D);
//        double gcd = Math.pow(fixedsens * (double) 0.6F + (double) 0.2F, 3.0D) * 8.0D;
//
//        final float yaw = (float) (previousRotation.x + (Math.ceil(((rotation.x - previousRotation.x) / gcd) / 0.15F) * gcd) * 0.15F);
//        final float pitch = (float) (previousRotation.y + (Math.ceil(((rotation.y - previousRotation.y) / gcd) / 0.15F) * gcd) * 0.15F);
//
//        return new Vector2f((rotation.x + yaw), (rotation.y + MathHelper.clamp(pitch, -90, 90)));
//    }
    ...
    public Vector2f smooth(final Vector2f lastRotation, final Vector2f targetRotation, final double speed) {
        ...
        final Vector2f fixedRotations = RotateTransaction.applySensitivityPatch(rotations);
