...
import minecraft.game.operation.combat.DDTBOT;
import minecraft.game.operation.combat.LegitATTACK;
...
import minecraft.game.operation.misc.NoInteract;
...
public class PlayerController {
    ...
    public void updateController() {
        // Проверяем, чтобы игрок был доступен и контроллер клиента был инициализирован
        if (mc.player != null && mc.playerController != null) {
            // Отправляем пакет для принудительной синхронизации
            mc.player.connection.sendPacket(new CPlayerPacket(mc.player.onGround));

            // Обновляем клиентский инвентарь
            mc.playerController.syncCurrentPlayItem();

            // Вызываем событие инвентаря для обновления локального клиента
    ...
    public void attackEntity(PlayerEntity playerIn, Entity targetEntity) {
        ...
        DDTBOT ddbot = AG.getInst().getModuleManager().getDDTBOT();
        LegitATTACK legit = AG.getInst().getModuleManager().getLegitATTACK();

        if (ddattack.getTarget() !=null || ddbot.getTarget() !=null || legit.getTarget() !=null) {
            if (ClickSettings.sprintType.is("Легитный")) {
            ...
            if (ClickSettings.sprintType.is("Пакетный")) {
                this.syncCurrentPlayItem();
