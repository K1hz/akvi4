...
import minecraft.game.operation.combat.Peregon;
        ...
        if (this.isAttachedToEntity()) {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    if (AG.getInst().getModuleManager().getElytraBooster().isEnabled()) {
                        boostedSpeed = AG.getInst().getModuleManager().getElytraBooster().boosterSpeed;
                    }

                    DDATTACK ddattack = AG.getInst().getModuleManager().getDDATTACK();

                    if (ddattack.isEnabled() && ddattack.getOptions().is("Коррекция движения").getValue() && this.boostedEntity instanceof ClientPlayerEntity) {
                        Vector3d vector3d;

                        if (AG.getInst().getModuleManager().getPeregon().isEnabled() && ddattack.getTarget() != null) {
                            // Если есть цель, использовать её параметры
                            vector3d = this.getVectorForRotation(DDATTACK.rotateVector.y, DDATTACK.rotateVector.x);
                        } else {
                            // Если цели нет, использовать направление взгляда игрока
                        ...
                        this.boostedEntity.setMotion(
                                vector3d1.add(
                                        vector3d.x * 0.1D + (vector3d.x * boostedSpeed - vector3d1.x) * 0.5D,
                                        vector3d.y * 0.1D + (vector3d.y * boostedSpeed - vector3d1.y) * 0.5D,
                                        vector3d.z * 0.1D + (vector3d.z * boostedSpeed - vector3d1.z) * 0.5D
                                )
                        );
                    } else {
                        // Если модуль отключён или настройки не требуют коррекции движения
