...
import minecraft.game.transactions.EventLoadWorld;
import minecraft.system.via.fixes.AttackOrder;
...
import minecraft.game.advantage.advisee.WHook;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public int leftClickDelayTimer;
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        ModuleManager moduleManager = minecraft.system.AG.getInst().getModuleManager();
    ...
    public void setDefaultMinecraftTitle() {
        ModuleManager functionRegistry = AG.getInst().getModuleManager();
        SelfDestruct selfDestruct = functionRegistry.getSelfDestruct();

        if (selfDestruct.unhooked) {
            this.mainWindow.setWindowTitle(getWindowTitle());
        } else {
            this.mainWindow.setWindowTitle("Aegis Client 1.16.5 (Discord: discord.gg/aegisclient), (Telegram: t.me/aegisclient)");
        }
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        minecraft.system.AG.getInst().getEventBus().post(eventTick);
...
//                this.player.swingArm(Hand.MAIN_HAND);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ModuleManager moduleManager = minecraft.system.AG.getInst().getModuleManager();
                                DDATTACK DDATTACK = moduleManager.getDDATTACK();
                                ...
                                boolean DDATTACKIsStateAndTargetNotNull = DDATTACK.isEnabled() && DDATTACK.getTarget() != null;

                                if (!DDATTACKIsStateAndTargetNotNull) {
                    ...
                    DDCOLDWN.ItemEnum itemEnum = minecraft.game.operation.combat.DDCOLDWN.ItemEnum.getItemEnum(itemstack.getItem());


                    if (!itemstack.isEmpty()) {
                        ActionResultType actionresulttype2;

                        if (DDCOLDWN.isEnabled() && itemEnum != null && DDCOLDWN.isCurrentItem(itemEnum) && player.getCooldownTracker().hasCooldown(itemEnum.getItem())) {
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            AG.getInst().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        minecraft.system.AG.getInst().getEventBus().post(new EventChangeWorld());
        minecraft.system.AG.getInst().getEventBus().post(new EventLoadWorld());
    }
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER && !AG.getInst().getModuleManager().getNoRender().isEnabled() && !AG.getInst().getModuleManager().getNoRender().element.is("Эффект свечения").getValue();
    }
